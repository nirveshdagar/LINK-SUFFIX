...
import CampaignProxySelector, { EMPTY_PROXY_CHOICE } from "./campaign-proxy-selector";
...
import CapacityRailLink from "@/components/capacity-rail-link";
import ProxyRailLink from "@/components/proxy-rail-link";
...
export default function ControlPanel() {
  ...
  const proxyProviderMode = proxyChoice.poolId ? "registry" : "legacy";
  ...
  const ready = connected && proxyChoice.ready && (proxyProviderMode === "registry" ? tier === "human" : proxyConfigured) && validUrl && authorized && (!browserTier || devicePool.length > 0) && (!burstMode || burstAuthorized) && (tier !== "human" || (!syncGoogleAds && !useScriptMesh) || Boolean(runCustomerId.trim() && runGoogleCampaignId.trim()));
  ...
  const campaignPayload = () => ({
      ...
      proxyProviderMode, proxyPoolId: proxyChoice.poolId, proxyProviderId: proxyChoice.providerId, proxyPort: proxyChoice.poolId ? 0 : proxyPort, customerId: runCustomerId, googleCampaignId: runGoogleCampaignId, loginCustomerId: runLoginCustomerId,
    ...
    setNotice(error instanceof Error ? error.message : "Unable to establish a secure session.");
        ...
        <section id="compose" className="compose-grid">
          <div className="composer">
            ...
            <div className="credential-note">Requested route: {selectedCountry?.name ?? geo.country}{geo.state ? ` / ${geo.state}` : ""}{geo.city ? ` / ${geo.city}` : ""}. Leave state and city blank for random country-wide residential exits. Timezone is resolved from the actual exit IP.</div>
          ...
          <aside className="launch-card">
            ...
            {proxyProviderMode === "legacy" && !proxyConfigured && <p className="credential-note">Enter your IPRoyal gateway, port, username, and base password above, then connect it to the local backend.</p>}
      ...
      />
        ...
        <section id="campaigns" className="run-deck saved-campaigns-deck">
          ...
          <div className="run-list">{filteredSavedCampaigns.length === 0 ? <div className="empty"><Activity size={24} /><p>{savedCampaigns.length ? "No campaigns match this search." : "No persistent campaigns saved."}</p></div> : visibleSavedCampaigns.map((item) => <article key={item.id}><span className={item.status === "running" ? "pulse" : "pulse ended"} /><div><b>{String(item.number).padStart(3, "0")} · {item.name}</b><small>Port {item.config.proxyPort} · {item.config.seedUrl}</small>{item.lastError && <small className="campaign-launch-error">Launch blocked: {item.lastError}{item.retryCount ? ` · retry ${item.retryCount}` : ""}</small>}</div><strong>{item.status.toUpperCase()}</strong><a href={`/runs/${encodeURIComponent(item.id)}/edit`}>Edit</a><button type="button" disabled={item.status === "running"} onClick={() => send({ type: "start_campaign", payload: { id: item.id } })}><Play size={14} />Start</button><button type="button" onClick={() => send({ type: "restart_campaign", payload: { id: item.id } })}><RefreshCw size={14} />Restart</button><button type="button" disabled={item.status !== "running" && item.status !== "queued"} onClick={() => send({ type: "stop_campaign", payload: { id: item.id } })}><CircleStop size={14} />Stop</button></article>)}</div>
