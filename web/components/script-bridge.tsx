      ...
      <div className="fleet-bottom-grid">
        ...
        <section className="fleet-script-card" aria-labelledby="fleet-script-title">
          ...
          <button className="bridge-primary-action" type="button" onClick={generateWorker} disabled={generating || !publicBaseUrl.trim() || !selectedShardId}>
            {generating ? "Generating v5 worker..." : selectedShard?.registered ? "Rotate token and regenerate v5 worker" : "Generate v5 worker for selected shard"}
          </button>
          <p className="fleet-script-warning">Generating again rotates the secret and immediately invalidates the older installed copy for this shard.</p>

          {generatedScript && generatedForShard === selectedShardId ? (
            <div className="fleet-script-output">
              <div><strong>{generatedForShard} · fleet-hourly-relay-v5</strong><button type="button" onClick={copyWorker}>Copy script</button><button className="bridge-secondary-action" type="button" onClick={() => setGeneratedScript("")}>Hide</button></div>
