...
type Provider = {
  ...
  authMode: string; rotationModes: string[]; secretConfigured: boolean; usernameTemplate?: string; passwordTemplate?: string;
...
export default function ProxyProviderDashboard() {
  ...
  async function post(action: string, payload: Record<string, unknown>, success: string) {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Operation failed"); return false; }
    ...
    <main className={`control-shell ${styles.shell}`}>
      ...
      <div className={`workspace ${styles.workspace}`}>
        ...
        <section className={styles.providerSection}>
          ...
          <div className={styles.providerGrid}>
            {overview.providers.map((item) => {
              const configured = item.enabled && (item.secretConfigured || item.authMode === "ip-allowlist");
              const healthy = configured && item.circuit?.state !== "open" && item.metrics.healthy === true && item.metrics.samples > 0;
              return <article className={styles.providerCard} key={item.providerId} data-health={healthy ? "healthy" : item.metrics.samples === 0 ? "unknown" : "alert"}>
                <div className={styles.cardTitle}><span className={styles.healthDot} /><div><strong>{item.name}</strong><small>{item.providerId} · {item.providerType}</small></div><em>{healthy ? "HEALTHY" : configured && !item.metrics.samples ? "READY / UNTESTED" : "CHECK"}</em></div>
                ...
                <div className={styles.cardActions}><button type="button" disabled={busy} onClick={() => setProvider({ providerId: item.providerId, name: item.name, providerType: item.providerType, protocol: item.protocol, gatewayHost: item.gatewayHost, gatewayPorts: item.gatewayPorts.join(","), authMode: item.authMode, usernameTemplate: item.usernameTemplate || "{username}", passwordTemplate: item.passwordTemplate || "{password}", rotationModes: item.rotationModes, username: "", password: "", token: "" })}>Edit provider</button><button type="button" disabled={busy} onClick={() => void post("set_provider_enabled", { providerId: item.providerId, enabled: !item.enabled }, item.enabled ? "Provider disabled safely." : "Provider enabled.")}>{item.enabled ? "Disable" : "Enable"}</button>{item.circuit?.state === "open" && <button type="button" disabled={busy} onClick={() => void post("reset_circuit", { providerId: item.providerId }, "Circuit reset; health probes may resume.")}>Reset circuit</button>}</div>
              </article>;
            })}
        ...
        <section className={styles.formGrid}>
          <form onSubmit={async (event) => { event.preventDefault(); const { username, password, token, ...definition } = provider; const saved = await post("save_provider", { provider: { ...definition, gatewayPorts: provider.gatewayPorts.split(","), capabilities: { country: true, state: true, city: true, asn: false, stickySession: true } }, secret: { username, password, token } }, "Provider and automatic pool saved securely."); if (saved) setProvider(current => ({ ...current, username: "", password: "", token: "" })); }}>
            <div className={styles.formTitle}><Plus /><div><p className="kicker">01 / Registry</p><h2>Add or edit a provider</h2></div></div>
            <button type="button" className={styles.secondary} onClick={() => setProvider({ providerId: "evomi", name: "EVOMI", providerType: "universal", protocol: "http", gatewayHost: "core-residential.evomi.com", gatewayPorts: "1000", authMode: "username-password", usernameTemplate: "{username}", passwordTemplate: "{password}", rotationModes: ["sticky-session"], username: "", password: "", token: "" })}>Use EVOMI residential preset</button>
            <p>Enter login details below. Saving also creates an automatic pool. EVOMI shares port 1000 across isolated browser sessions; 1001 and 1002 are different protocols, not extra campaign ports.</p>
            ...
            <details><summary>Advanced: credential templates (not login details)</summary><p>Keep {`{username}`} and {`{password}`} placeholders here. EVOMI session and location parameters are generated automatically.</p><div className={styles.two}><label>Username template<input value={provider.usernameTemplate} onChange={(e) => setProvider({ ...provider, usernameTemplate: e.target.value })} /></label><label>Password template<input value={provider.passwordTemplate} onChange={(e) => setProvider({ ...provider, passwordTemplate: e.target.value })} /></label></div></details>
            <div className={styles.secret}><KeyRound size={17} /><label>Proxy username<input autoComplete="off" value={provider.username} onChange={(e) => setProvider({ ...provider, username: e.target.value })} /></label><label>Proxy password<input type="password" autoComplete="new-password" value={provider.password} onChange={(e) => setProvider({ ...provider, password: e.target.value })} /></label><label>Token<input type="password" autoComplete="new-password" value={provider.token} onChange={(e) => setProvider({ ...provider, token: e.target.value })} /></label></div>
          ...
          <div className={styles.stack}>
            <p>Default pools are created automatically. Use custom pools only for a provider with separate endpoint groups. Campaign proxy selection is also available on each campaign edit page.</p>
            <details><summary>Advanced: custom endpoint pools</summary>
            <form onSubmit={(event) => { event.preventDefault(); void post("save_pool", { pool: { ...pool, endpointPorts: pool.endpointPorts.split(",") } }, "Endpoint pool saved."); }}>
              <div className={styles.formTitle}><Network /><div><p className="kicker">02 / Leasing</p><h2>Custom endpoint pool</h2></div></div>
            ...
            <form onSubmit={(event) => { event.preventDefault(); void post("assign_policy", { policy }, "Campaign proxy policy assigned."); }}>
              ...
              <div className={styles.two}><label>Primary provider<select required value={policy.primaryProviderId} onChange={(e) => setPolicy({ ...policy, primaryProviderId: e.target.value, primaryPoolId: overview.pools.find((item) => item.providerId === e.target.value && item.enabled)?.poolId || "", rotationMode: overview.providers.find((item) => item.providerId === e.target.value)?.rotationModes[0] || "provider-managed" })}><option value="">Select provider</option>{overview.providers.filter((item) => item.enabled).map((item) => <option key={item.providerId} value={item.providerId}>{item.name}</option>)}</select></label><label>Endpoint pool<select required value={policy.primaryPoolId} onChange={(e) => setPolicy({ ...policy, primaryPoolId: e.target.value })}><option value="">Select pool</option>{policyPools.map((item) => <option key={item.poolId} value={item.poolId}>{item.name}</option>)}</select></label></div>
