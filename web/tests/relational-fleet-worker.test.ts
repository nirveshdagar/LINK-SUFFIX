...
import {
  buildRelationalFleetV9Worker,
  RELATIONAL_FLEET_WORKER_VERSION,
} from "../lib/relational-fleet-worker.ts";

test("generates the relational v9 adaptive two-phase Fleet worker", () => {
  const script = buildRelationalFleetV9Worker(
    "https://bridge.example/api/script-bridge/jobs",
    "secret-token",
    "mcc-1000000000-001",
  );

  assert.equal(RELATIONAL_FLEET_WORKER_VERSION, "fleet-two-phase-adaptive-relay-v9");
