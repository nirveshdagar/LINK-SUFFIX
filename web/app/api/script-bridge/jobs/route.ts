...
const ACTIVE_PROTOCOL = "fleet-two-phase-resilient-relay-v11";
const ADAPTIVE_PROTOCOLS = new Set([ACTIVE_PROTOCOL, "fleet-callback-resilient-relay-v10", "fleet-two-phase-adaptive-relay-v9"]);
const LEGACY_PROTOCOLS = new Set(["fleet-callback-resilient-relay-v10", "fleet-two-phase-adaptive-relay-v9", "fleet-two-phase-durable-relay-v8", "fleet-two-phase-hot-add-relay-v7", "fleet-hot-add-relay-v6", "fleet-hourly-relay-v5"]);
...
async function lease(request: Request, input: Record<string, unknown>) {
  ...
  if (!protocolAllowed(input.protocol) || !contractAllowed(input.contract)) {
    return NextResponse.json({ error: "Outdated Fleet worker. Regenerate and install the selected shard's v11 two-phase resilient script." }, { status: 409 });
  }
    ...
    return NextResponse.json(
      {
        ...
        ...(await bridgeShardManifest(shardId, {
          ...
          protocol,
          preview,
          adaptive: ADAPTIVE_PROTOCOLS.has(protocol),
        })),
        protocol,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "window") {
      if (!protocolAllowed(input.protocol) || !contractAllowed(input.contract)) {
        return withRateLimitHeaders(NextResponse.json({ error: "Outdated Fleet worker. Regenerate and install the selected shard's v11 two-phase resilient script." }, { status: 409 }), limit);
      }
      const shardId = String(input.shardId || input.shard || "default").trim();
      ...
      const recovered = await bridgeActiveExecutionWindow(shardId);
      return withRateLimitHeaders(NextResponse.json({ ok: true, ...recovered, serverTime: new Date().toISOString() }, { headers: { "cache-control": "no-store" } }), limit);
    }
    if (action === "complete") {
      if (!protocolAllowed(input.protocol) || !contractAllowed(input.contract)) {
        return withRateLimitHeaders(NextResponse.json({ error: "Outdated Fleet worker. Regenerate and install the selected shard's v11 two-phase resilient script." }, { status: 409 }), limit);
