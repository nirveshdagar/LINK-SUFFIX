...
export async function GET(_req: NextRequest) {
  ...
  if (runId) {
    const local = readRunFile(runId, 'mismatches.csv')
    if (local !== null) {
      return new Response(local || 'no mismatches yet', {
        status: 200,
        headers: {
    ...
    const upstreamRes = upstream ? await fetch(`${upstream}/mismatches?runId=${encodeURIComponent(runId)}`, { cache: 'no-store' }).catch(() => null) : null;
    if (!upstreamRes) {
      return new Response('no mismatches yet', {
        status: 200,
        headers: {
          'content-type': 'text/plain; charset=utf-8',
          'cache-control': 'no-store',
  ...
  const upstreamRes = upstream ? await fetch(`${upstream}/mismatches`, { cache: 'no-store' }).catch(() => null) : null;
  if (!upstreamRes) {
    return new Response('no mismatches yet', { status: 200 });
  }
  if (upstreamRes.status === 404) {
    return new Response('no mismatches yet', {
      status: 200,
